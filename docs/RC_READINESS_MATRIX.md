...
| `/vp`, `/privat`, `/privatadmin`, `/home`, `/sethome` are registered | `PARTIAL` | Bootstrap command probes passed; docs record command availability. | Run commands as real player/admin in `docs/MANUAL_PLAYER_SMOKE_RUNBOOK.md`. |
...
| API lets external transfer plugin read/move/relocate without DB/reflection | `PROVEN` | `VibePrivateAPI` and thin `VibeRegionGuardApi` facade expose reads, validation, move/relocate primitives and events. Architecture docs ban SQL/reflection for integrations. | Keep transfer orchestration outside this plugin; add integration helper only if manual smoke needs it. |
